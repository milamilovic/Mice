...
class CvorListe(object):
    def __init__(self, vrednost, sledeci=None):
...
class Jednostruka(object):
    ...
    def ukloniPoslednjeg(self):
        if self._duzina == 0:
            ...
            cvoric = self._head
            while cvoric._sledeci != self._tail:
                cvoric = cvoric._sledeci
            self._tail = cvoric
            cvoric._sledeci = None
            self._duzina -= 1   
    def Prvi(self):
